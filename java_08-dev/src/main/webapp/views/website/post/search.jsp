            ...
            <div class="row">
                <div class="col-lg-8 mb-5 mb-lg-0">
                    <div class="blog_left_sidebar">
                        <div>
                            <h3 class="mb-4">Kết quả tìm kiếm: ${search}</h3>
                        </div>
                            ...
                            <c:forEach var="item" items="${posts.content}">
                                <article class="blog_item">
                                    <div class="p-3 d-flex blog_details">
                                        <div class="mr-3 py-2">
                                            <a href="/users/wall/${item.getUserID().ID}/${item.getUserID().username}" class="d-block" style="height: 3.5rem; width: 3.5rem;">
                                        ...
                                        <div>
                                            <a href="/users/wall/${item.getUserID().ID}/${item.getUserID().username}" class="d-block m-0">
                                ...
                                <nav class="blog-pagination justify-content-center d-flex">
                                    <ul class="pagination">
                                        <c:if test="${posts.number > 0}">
                                            <li class="page-item">
                                                <a href="/posts/search?search=${search}&page=${posts.number}" class="page-link" aria-label="Previous">
                                        ...
                                        <c:forEach var="i" begin="1" end="${posts.totalPages }">
                                            <c:if test="${i-1 != posts.number}">
                                                <li class="page-item">
                                                    <a href="/posts/search?search=${search}&page=${i}" class="page-link">${i}</a>
                                                </li>
                                            </c:if>
                                            <c:if test="${i-1 == posts.number}">
                                        ...
                                        <c:if test="${posts.number + 1 ne posts.totalPages}">
                                            <li class="page-item">
                                                <a href="/posts/search?search=${search}&page=${posts.number+2}" class="page-link" aria-label="Next">
