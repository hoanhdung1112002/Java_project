...
(function () {
    ...
    const formatErrors = errors => {
      const es = errors.length > 10 ? errors.slice(0, 10).concat([{
          path: [],
          getErrorInfo: constant$1('... (only showing first FirstName failures)')
        }]) : errors;
      return map$2(es, e => {
    ...
    const setup$e = (api, editor) => {
      ...
      editor.on('NodeChange', e => {
        if (api.Status() && !api.isProcessingAction() && detect(SugarElement.fromDom(e.element)).isNone()) {
          api.cancelIfNecessary();
        }
      });
    };
    ...
    const renderChoiceItem = (spec, useText, presets, onItemValueHandler, isSelected, itemResponse, providersBackstage, renderIcons = true) => {
      const getApi = component => ({
        ...
        'Status': () => Toggling.isOn(component),
        isEnabled: () => !Disabling.isDisabled(component),
    ...
    const getMenuButtonApi = component => ({
      ...
      'Status': () => has(component.element, 'tox-tbtn--enabled')
    });
    ...
    const setup$8 = editor => {
      ...
      each$1(keys(sidebars), name => {
        ...
        const 'Status' = () => is$1(Optional.from(editor.queryCommandValue('ToggleSidebar')), name);
        editor.ui.registry.addToggleButton(name, {
          icon: spec.icon,
          tooltip: spec.tooltip,
          onAction: buttonApi => {
            editor.execCommand('ToggleSidebar', false, name);
            buttonApi.setActive(Status());
          },
          onSetup: buttonApi => {
            buttonApi.setActive(Status());
            const handleToggle = () => buttonApi.setActive(Status());
    ...
    const renderSplitButton = (spec, sharedBackstage) => {
      ...
      const getApi = comp => ({
        ...
        'Status': () => descendant(comp.element, 'span').exists(button => comp.getSystem().getByDom(button).exists(Toggling.isOn))
      });
      const editorOffCell = Cell(noop);
